<div class="admin-panel" id="panelOffers">
    <div class="admin-card">
        <div class="admin-card-header">
            <h3>إدارة العروض والباقات</h3>
...
window.handleOfferSubmit = function(e) {
    ...
    // Normalize checkboxes for Laravel boolean validation
    const isFeatured = $('#offerIsFeatured').is(':checked') ? '1' : '0';
    const isActive = $('#offerIsActive').is(':checked') ? '1' : '0';
    
    // Remove if already in serialized array to avoid duplicates, then push new values
    ...
    $.ajax({
        ...
        success: function() {
            ...
            closeOfferModal();
            window.location.hash = 'panelOffers';
            location.reload();
        },
        error: function(xhr) {
            if (xhr.status === 422) {
                showRemoteErrors(xhr.responseJSON.errors, form);
                showNotif('يرجى التحقق من البيانات المدخلة', 'error');
            } else {
                showNotif('حدث خطأ في النظام', 'error');
            }
